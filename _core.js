...
function showRoadmapPanel(panel) {
  const monthRows = [
    ['now','Неделя 1','Фундамент','Рефакторинг в модули, склад/носильщики, цепочки крафта, стены+ворота+A*, визуал-пасс 1 (частицы, береговая линия — уже начато)'],
    ['soon','Неделя 2','Контент','Лошади/ранчо, мебель/комнаты, боевая глубина, караваны/квесты, аудио-пасс, визуал-пасс 2'],
    ['soon','Неделя 3','Онлайн-фундамент','Детерминизм+команды, save v2, бэкенд-скелет, облачные сейвы'],
    ['soon','Неделя 4','ALPHA','Авторитетный сервер, кооп на двоих по интернету, облачные сейвы, дизайн глобального мира'],
  ];
  const rows = [
    ['done','v1.0','Фундамент','Карта, ИИ пешек, расписание, приоритеты, постройки, наука, животные, погода, сезоны, события, сохранение'],
    ['done','v1.1','Графика','Рисованные ковбои с анимацией, нарисованные здания, тени, цикл день/ночь с освещением'],
    ['done','v1.2','Играбельность','Цель с прогресс-баром, туториал, пауза и скорости, спокойный первый день'],
    ['done','v1.3','Меню','Починен автобой, правый клик = постоянная цель атаки, главное меню с роадмапом'],
    ['done','v1.4','Бой','Огнестрел с пулями и точностью, укрытия, типы врагов (стрелок/головорез/главарь), самозащита'],
    ['done','v1.5','Камера и удобство','Миникарта, зум колесом и кнопками, полный экран, обычный курсор'],
    ['done','v1.6','Жизнь колонии','Характеры (10 трейтов), отношения и ссоры, болезни с лечением'],
    ['now','v1.7','Полировка','Звук (WebAudio), достижения, тач-управление, растущие волны бандитов'],
  ];
  panel.innerHTML = `
    ...
    <div style="background:#1a1610;border:1px solid #3a3320;border-radius:6px;padding:8px 10px;margin-bottom:12px">
      <b style="color:#e8c97e">🚩 Стадия: PRE-ALPHA</b>
      <div style="color:#999;font-size:11px;margin-top:3px">Прототип ядра готов. Дальше — месячный план к ALPHA (онлайн + кооп). Полные доки: <b>docs/</b></div>
    </div>
    <h3 style="margin-top:0">📅 Месячный план</h3>
    ${monthRows.map(([st,ver,name,desc])=>{
      const tag = st==='now'?'<span class="rm-tag tag-now">СЕЙЧАС</span>':'<span class="rm-tag tag-soon">СКОРО</span>';
      const cls = st==='now'?'rm-now':'rm-soon';
      return `<div class="rm-row">${tag}<b class="${cls}">${ver} — ${name}</b><div style="color:#888;font-size:11px;margin-top:2px">${desc}</div></div>`;
    }).join('')}
    <h3>🧱 История прототипа</h3>
    <p style="font-size:11px;color:#888">Каждая сборка — отдельный файл в <b>versions/</b>, играй в любую.</p>
    ${rows.map(([st,ver,name,desc])=>{
      const tag = st==='done'?'<span class="rm-tag tag-done">ГОТОВО</span>'
                : st==='now'?'<span class="rm-tag tag-now">СЕЙЧАС</span>'
                : '<span class="rm-tag tag-soon">СКОРО</span>';
      const cls = st==='done'?'rm-done':st==='now'?'rm-now':'rm-soon';
      return `<div class="rm-row">${tag}<b class="${cls}">${ver} — ${name}</b><div style="color:#888;font-size:11px;margin-top:2px">${desc}</div></div>`;
    }).join('')}
    <h3>Последнее обновление (Pre-Alpha 0.8.0 — старт визуал-пасса 1)</h3>
    <ul>
      <li>✨ <b>Частицы</b> — пыль и щепки при рубке, осколки при добыче камня, дымок при стройке, вспышки выстрелов</li>
      <li>🌊 <b>Береговая линия</b> — пенная кромка на стыке воды и суши</li>
      <li>📚 <b>Документация и месячный план</b> — docs/ (дизайн, архитектура, визуал, звук, мультиплеер)</li>
      <li>🚩 Проект официально помечен как <b>PRE-ALPHA</b> с планом до ALPHA (онлайн + кооп)</li>
    </ul>
  `;
}
...
function showHowtoPanel(panel) {
  panel.innerHTML = `
    ...
    <ul>
      ...
      <li>🏗️ <b>Выбери постройку снизу</b> → кликни на карту. Зелёная зона = можно ставить.</li>
