...
- Последний локальный срез: `v1.40 - Homestead Comfort Score`.
- Базовая игра: `frontier.html`, сборка через `frontier.template.html` + `_core.js`.
- Текущий `GAME_VERSION`: `1.40`.
- Harness: Scenarios A-AF должны быть зелёные; перед push обязательно `node _harness.js`.
