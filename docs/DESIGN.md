...
## ИССЛЕДОВАНИЕ: СТРОЙКА RIMWORLD (и как делаем у нас)

**Суть RimWorld-стройки (что копируем по смыслу):**
1. Architect-меню по категориям (Структуры/Мебель/Полы/Производство/Зоны/...). [у нас есть]
2. Чертёж -> подвоз материалов -> постройка; МАТЕРИАЛ влияет на статы (HP/красота/работа). [у нас: чертёж+подвоз есть; материал=отдельные постройки (дерев./камен. стена), не общий «stuff»]
3. Стены 1-клеточные кистью из материала, образуют комнаты. [есть]
4. Двери из материала, авто-открытие для своих. [у нас: ворота=дверь]
5. Полы кистью; материал влияет на красоту/скорость. [есть: дерево/камень/дорога]
6. Крыша авто над замкнутым; зоны крыши. [есть авто; зоны крыши — в очереди]
7. Комната авто-распознаётся и ТИПИЗИРУЕТСЯ по содержимому: спальня(кровать)/столовая(стол)/КУХНЯ(плита)/мастерская(верстак)/склад/госпиталь/тюрьма/тронный зал. [у нас: спальня/столовая/жилая/декор — РАСШИРЯЕМ]
8. Стат комнаты: красота/простор/чистота/«впечатляемость» -> настроение. [у нас: уют/качество стен/пол — углубим]
9. КАЧЕСТВО мебели (awful..legendary) от навыка строителя -> красота/функция. [у нас навыки есть, качество — TODO]
10. Деконструкция (возврат части материалов), переустановка мебели. [деконструкция есть; переустановка — TODO]
11. Температура (стены+крыша+обогрев). [TODO, крупное]

**Что добавляем СВОЁ (Frontier-идентичность, тот же смысл — другая тема):**
- Frontier-типы комнат: «САЛУН-ЗАЛ» (салун внутри => место сбора/слухи), «БАРАК» (2+ кровати => жильё ковбоев), «контора шерифа»/«камера» (Phase 4), «веранда».
- Материалы Дикого Запада: саман/глинобит (дёшев в пустыне), брус, песчаник — материал привязан к биому (адобе в desert).
- Признак «ДОМ»: замкнутая стенами комната + дверь (ворота) => жилое помещение (как ты и сказал).

**Что делаю этим срезом (v2.10):** расширяю авто-типы комнат (кухня/мастерская/лаборатория + наши салун-зал/барак), добавляю признак «ДОМ», вывожу тип ярче. Материалы-«stuff», качество мебели, температура — следующими срезами по этому же плану.
