...
## Очередь (новый план — Phase 1: Playable Core)

1. ✅ Реворк времени суток (v1.79). Дальше: эффекты ночи на работу/бой/животных (срез 1b).
2. Местность влияет на скорость движения (фундамент под систему веса Phase 2).
3. Базовая система огня: распространение по горючему + тушение.
