...
1. Вода как реальное препятствие: A* через мелководье/броды, риск от навыка, груза и погоды.
2. Frontier-экосистема: животные/опасности/фуражировка зависят от биома и погоды.
3. Здоровье по частям тела: раны, лечение и долгие последствия без мгновенной смерти.
...
- v2.03 - Swimming foundation (Claude-local, Phase 3). Каркас плавания: `swimSpeedMul(p)` и `canSwim(p)`; A* воду пока не открывает. Scenario CF. Harness A-CF (84) green.
